        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, i) => (
            <motion.div key={item.handle} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}>
              ...
              <p className="text-xs text-muted tracking-widest uppercase mb-1">NÜRHAUS</p>
  ...
  return (
    <div className="page-enter">
      <div className="site-max site-px pt-16 pb-12 border-b border-hairline">
        <motion.p className="eyebrow mb-3" variants={fadeUp} initial="hidden" animate="visible">My Account</motion.p>
        <motion.h1
          className="font-display text-[2.5rem] md:text-[4rem] leading-tight tracking-tight"
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.1, duration: 0.8, ease: [0.22,1,0.36,1] }}
        >
          Welcome back, <em>Amira.</em>
        </motion.h1>
      </div>
